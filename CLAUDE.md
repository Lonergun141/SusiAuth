...
> **API prefix:** `config/urls.py` mounts the `api_v1` NinjaAPI under **`/api/v1`**. First-party
> routes include `/api/v1/auth/login`, `/api/v1/health`, and
> `/api/v1/.well-known/jwks.json`. OAuth/OIDC remains separately mounted under `/o/`.
...
src/authsvc/
  ...
  apps/
    ...
    audit/              immutable AuditEvent model/service + DOT client signals
...
**Health endpoints:** `/api/v1/health` (basic), `/api/v1/health/live` (liveness, no deps),
`/api/v1/health/ready` (readiness — checks DB + JWT signing keys, returns 503 if unavailable).
...
a hand-rolled server (per the brief). First-party auth (`/api/v1/auth/*`, RS256 JWTs) is unchanged; DOT
...
- **Known limitations:** two JWKS endpoints exist (`/api/v1/.well-known/jwks.json` for first-party,
  `/o/.well-known/jwks.json` for OIDC) — same key, different URLs; no consent-screen customization yet.

## MFA (`apps/mfa`) — TOTP

- **Enroll:** `POST /api/v1/auth/mfa/setup` (auth) returns a secret + `otpauth://` URI (render as QR);
  `POST /api/v1/auth/mfa/confirm` verifies the first code, sets `User.mfa_enabled`, and returns
...
- **Login challenge:** when `mfa_enabled`, `/api/v1/auth/login` returns `{mfa_required: true, mfa_token}`
  ...
  `POST /api/v1/auth/mfa/verify {mfa_token, code}` accepts a TOTP **or** a recovery code and issues tokens.
...
- **Webhooks:** `POST /api/v1/webhooks/resend` verifies the **Svix signature over the raw body** before
...
## Audit events (`apps/audit`)

- `AuditEvent` stores immutable scalar actor/target snapshots, event/result, timestamp, request ID,
  IP, user agent, and sanitized structured metadata. It deliberately has no mutable foreign keys.
- Model, queryset, and admin rules block updates and deletes. Call `record_event()` synchronously at
  the security action boundary; sensitive metadata keys are recursively redacted.
- Current auth, refresh/reuse, session, password, MFA, notification, webhook, and DOT OAuth-client
  lifecycle paths emit events. Future administrative capabilities already have reserved event types.
- In PostgreSQL deployments, use separate migration and runtime roles where possible. Grant the
  runtime role `SELECT, INSERT` on the audit table but not `UPDATE` or `DELETE`; retention jobs should
  use a separately controlled maintenance role.
...
Redis must be running for distributed rate limiting on non-test settings. Celery defaults to Redis
DB 0; the Django cache and rate-limit counters default to DB 1.
...
- **Rate-limit counters use Redis DB 1** via `REDIS_CACHE_URL`; Celery uses DB 0. Tests deliberately
  replace the cache with `LocMemCache` and disable throttling to remain dependency-free.
